...
export const exportGlobalData = (
    ...
    // 6. HISTORY CHARTS DATA
    // 6.1 Cumulative P&L
    const sortedPnl = [...pnlData].sort((a, b) => new Date(a.sellDate).getTime() - new Date(b.sellDate).getTime());
    let cumulative = 0;
    const cumulativeData = sortedPnl.map(p => {
        let rate = 1;
        const m = (p.market || '').toUpperCase().trim();
        if (m === 'HK') rate = marketConstants.exg_rate;
        else if (m === 'SG') rate = marketConstants.sg_exg;
        ...
        return {
            Date: p.sellDate,
            'Daily P&L (USD)': Math.round(pnlUsd),
            'Cumulative P&L (USD)': Math.round(cumulative)
        };
    });

    // 6.2 Stock Stats
    const statsMap = new Map<string, { count: number, totalPnl: number, totalVol: number }>();
    pnlData.forEach(p => {
        ...
        const pnlUsd = p.realizedPnL / rate;
        const volUsd = Math.abs(p.totalBuy) / rate;
        
        const current = statsMap.get(stock) || { count: 0, totalPnl: 0, totalVol: 0 };
        current.count += 1;
        current.totalPnl += pnlUsd;
        current.totalVol += volUsd;
        statsMap.set(stock, current);
    });
    const stockStatsData = Array.from(statsMap.entries()).map(([stock, { count, totalPnl, totalVol }]) => ({
        Stock: stock,
        'Trade Count': count,
        'Avg P&L (USD)': Math.round(totalPnl / count),
        'Avg Trade Amt (USD)': Math.round(totalVol / count)
    })).sort((a, b) => b['Trade Count'] - a['Trade Count']);

    // 6.3 Holding Period
    const buckets: Record<string, number> = { '0-7 Days': 0, '8-30 Days': 0, '1-3 Months': 0, '3-6 Months': 0, '6-12 Months': 0, '> 1 Year': 0 };
    pnlData.forEach(p => {
    ...
    const holdingData = Object.entries(buckets).map(([range, count]) => ({ Range: range, Count: count }));

    const histChartsWs = XLSX.utils.json_to_sheet(cumulativeData);
    formatWorksheet(histChartsWs, cumulativeData);
    XLSX.utils.book_append_sheet(workbook, histChartsWs, "History - Cumulative P&L");
    
    const stockStatsWs = XLSX.utils.json_to_sheet(stockStatsData);
    formatWorksheet(stockStatsWs, stockStatsData);
    XLSX.utils.book_append_sheet(workbook, stockStatsWs, "History - Stock Stats");

    const holdingWs = XLSX.utils.json_to_sheet(holdingData);
    formatWorksheet(holdingWs, holdingData);
    XLSX.utils.book_append_sheet(workbook, holdingWs, "History - Holding Dist");
